...
class _FeatureTogglePageState extends State<FeatureTogglePage> {
  ...
  void _onToggleChanged(FeatureToggleModel toggle, bool value) {
    if (toggle.isLocked) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Modul ${toggle.name} adalah modul inti dan tidak dapat dimatikan.',
          ),
        ),
      );
      return;
    }
    context.read<FeatureToggleBloc>().add(
      UpdateFeatureToggle(key: toggle.key, isActive: value),
    );
  }
        ...
        child: Scaffold(
          ...
          body: BlocBuilder<FeatureToggleBloc, FeatureToggleState>(
            builder: (context, state) {
              if (state is FeatureToggleInitial ||
                  state is FeatureToggleLoading) {
                return const Center(child: CircularProgressIndicator());
              } else if (state is FeatureToggleLoaded) {
                if (state.toggles.isEmpty) {
                  return const Center(
                    child: Text('Belum ada pengaturan fitur.'),
                  );
                }
  ...
  Widget _buildModuleCard(
    ...
    FeatureToggleModel module,
    FeatureToggleLoaded state,
  ) {
    final bool isUpdating =
        state is FeatureToggleUpdating && state.updatingKey == module.key;
  ...
  Widget _buildChildTile(
    FeatureToggleModel child,
    FeatureToggleModel module,
    FeatureToggleLoaded state,
  ) {
    final bool isChildUpdating =
        state is FeatureToggleUpdating && state.updatingKey == child.key;
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              title: const Text(
                'Jenis Utilitas Aktif',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14),
              ),
              subtitle: const Text(
                'Pilih minimal satu jenis utilitas yang akan dipantau setiap bulan.',
                            ...
                            onSelected: isSaving
                                ...
                                : (selected) {
                                    ...
                                    context.read<SettingBloc>().add(
                                      UpdateSettingsEvent({
                                        'pengeluaran_tetap_jenis_aktif': newSet
                                            .toList(),
                                      }),
                                    );
                                  },
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text(
                'Pilih metode yang tersedia untuk penghuni. Toggle tersimpan otomatis.',
                style: TextStyle(fontSize: 11),
              ),
            ),
            Divider(height: 1, color: borderColor),
            Padding(
              ...
              child: BlocConsumer<PaymentMethodSettingCubit, PaymentMethodSettingState>(
                ...
                builder: (context, pmState) {
                  if (pmState is PaymentMethodSettingInitial ||
                      pmState is PaymentMethodSettingLoading) {
                    return const Center(
                      child: Padding(
                              ...
                              onChanged: isSavingPm
                                  ...
                                  : (val) {
                                      if (!val &&
                                          ...
                                          method.enabled) {
                                        _showWarningSnackBar(
                                          'Minimal satu metode harus aktif selama Midtrans dinyalakan.',
                                        );
                                        return;
                                      }
                                      final enabledCodes = methods
                                          ...
                                          .toList();
                                      context
                                          .read<PaymentMethodSettingCubit>()
                                          .save(enabledCodes);
                                    },
                            ),
                          );
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text(
                'Atur siapa yang menanggung biaya transaksi dan besaran tarifnya.',
                style: TextStyle(fontSize: 11),
              ),
            ),
            Divider(height: 1, color: borderColor),
            Padding(
              ...
              child: BlocConsumer<MidtransFeeCubit, MidtransFeeState>(
                      ...
                      SnackBar(
                        content: Row(
                          children: [
                            ...
                            const SizedBox(width: 10),
                            Text(
                              'Pengaturan biaya Midtrans disimpan',
                ...
                builder: (context, state) {
                  if (state is MidtransFeeInitial ||
                      state is MidtransFeeLoading) {
                    return const Center(
                      child: Padding(
                        padding: EdgeInsets.all(16),
                        child: CircularProgressIndicator(),
                  ...
                  final config = state is MidtransFeeLoaded
                      ? state.config
                      : state is MidtransFeeSaving
                      ? state.config
                      : state is MidtransFeeSaved
                      ? state.config
                      : null;
...
class _MidtransFeeSettingBody extends StatefulWidget {
  ...
  @override
  State<_MidtransFeeSettingBody> createState() =>
      _MidtransFeeSettingBodyState();
}

class _MidtransFeeSettingBodyState extends State<_MidtransFeeSettingBody> {
  late Map<String, TextEditingController> _controllers;
  final TextEditingController _simulationCtrl = TextEditingController(
    text: '100000',
  );
      ...
      children: [
        ...
        Row(
          children: [
            _BearerOption(
              ...
              description:
                  'Biaya dipotong dari penerimaan wisma. Penghuni bayar sesuai tagihan.',
              icon: Icons.store_outlined,
              selected: !isCustomer,
              accentColor: primaryColor,
              onTap: isSaving
                  ? null
                  : () => cubit.save(_buildUpdatedConfig('merchant')),
            ),
            const SizedBox(width: 8),
            _BearerOption(
              isDark: isDark,
              label: 'Penghuni',
              description:
                  'Biaya ditambahkan ke tagihan penghuni saat checkout.',
              icon: Icons.person_outline,
              selected: isCustomer,
              accentColor: warningColor,
              onTap: isSaving
                  ? null
                  : () => cubit.save(_buildUpdatedConfig('customer')),
            ),
          ],
        ),
                ...
                child: RichText(
                  text: TextSpan(
                    ...
                    children: [
                      const TextSpan(
                        text:
                            'Tarif di bawah sesuai ketentuan resmi Midtrans. ',
                      ),
                      TextSpan(
                        text:
                            'Jangan ubah kecuali ada pembaruan tarif dari Midtrans.',
                        style: TextStyle(
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: isSaving
                ? null
                : () => cubit.save(_buildUpdatedConfig(config.bearer)),
            icon: isSaving
                ? const SizedBox(
                    width: 14,
                    height: 14,
  ...
  Widget _buildSimulationTable(
    ...
    final nominal =
        double.tryParse(
          _simulationCtrl.text.replaceAll('.', '').replaceAll(',', ''),
        ) ??
        0;
    ...
    if (nominal <= 0) {
      return Container(
        ...
        child: Center(
          child: Text(
            'Masukkan nominal di atas untuk melihat simulasi.',
      ...
      child: Column(
        children: [
          ...
          ...config.fees.asMap().entries.map((entry) {
            ...
            final isLast = index == config.fees.length - 1;
            final rawVal =
                double.tryParse(_controllers[fee.key]?.text ?? '') ?? 0;
            final feeAmt = fee.type == 'flat'
                ? rawVal
                : (nominal * rawVal / 100).roundToDouble();
