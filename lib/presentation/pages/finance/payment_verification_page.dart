...
class _PaymentVerificationPageState extends State<PaymentVerificationPage> {
  ...
  Color _methodColor(String m) => m == 'midtrans' ? Colors.indigo.shade600 : Colors.teal.shade600;
  ...
  List<PaymentEntity> _filtered(List<PaymentEntity> all) {
    ...
    return all.where((p) =>
        ...
        p.invoiceId.toString().contains(q)).toList();
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('ID #${payment.transactionId ?? payment.id}', style: TextStyle(fontSize: 12, color: Colors.grey.shade500, fontWeight: FontWeight.normal)),
          ])),
        ]),
              ...
              Container(
                ...
                child: Row(children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(formatRupiah(payment.amount), style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  ])),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text('Metode', style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
                    Text(_methodLabel(payment.paymentMethod), style: TextStyle(fontSize: 13, color: _methodColor(payment.paymentMethod), fontWeight: FontWeight.w600)),
                    Text('Invoice #${payment.invoiceId}', style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
                  ]),
                ]),
              ),
              ...
              Container(
                ...
                child: (payment.paymentProofUrl != null && payment.paymentProofUrl!.isNotEmpty)
                    ? ClipRRect(
                        ...
                        child: Image.network(
                          payment.paymentProofUrl!, fit: BoxFit.contain,
                          loadingBuilder: (_, child, progress) => progress == null ? child : const Center(child: CircularProgressIndicator()),
                          errorBuilder: (_, __, ___) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.broken_image_outlined, size: 40, color: Colors.grey.shade400), const SizedBox(height: 8), Text('Gambar tidak dapat dimuat', style: TextStyle(color: Colors.grey.shade500))])),
                        ),
                      )
                    : Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.image_not_supported_outlined, size: 40, color: Colors.grey.shade400), const SizedBox(height: 8), Text('Tidak ada bukti transfer', style: TextStyle(color: Colors.grey.shade500))])),
              ),
              const SizedBox(height: 14),
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton.icon(
            ...
            onPressed: () {
              if (reasonCtrl.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Alasan refund wajib diisi!'))); return; }
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.center, children: [
                const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Verifikasi Pembayaran', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
                  SizedBox(height: 6),
                  Text('Tinjau dan konfirmasi bukti transfer pembayaran dari penghuni.', style: TextStyle(fontSize: 14, color: Colors.grey)),
                ]),
                OutlinedButton.icon(
              ...
              BlocBuilder<PaymentVerificationBloc, PaymentVerificationState>(
                ...
                builder: (ctx, state) {
                  ...
                  final manual   = all.where((p) => p.paymentMethod != 'midtrans').length;
                  final midtrans = all.where((p) => p.paymentMethod == 'midtrans').length;
                  ...
                  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    // ── Summary cards ────────────────────────────────
                    Row(children: [
                      Expanded(child: _summaryCard('Menunggu Verifikasi', '${all.length} Transaksi', 'Perlu tindak lanjut admin', Icons.hourglass_top, Colors.orange.shade700, Colors.orange.shade50)),
                      const SizedBox(width: 16),
                      Expanded(child: _summaryCard('Transfer Manual', '$manual Transaksi', 'Upload bukti transfer', Icons.account_balance, Colors.teal.shade700, Colors.teal.shade50)),
                      const SizedBox(width: 16),
                      Expanded(child: _summaryCard('Midtrans / QRIS', '$midtrans Transaksi', 'Pembayaran digital', Icons.qr_code, Colors.indigo.shade700, Colors.indigo.shade50)),
                      const SizedBox(width: 16),
                      Expanded(child: _summaryCard('Total Nominal', formatRupiah(totalNominal), 'Dari semua transaksi pending', Icons.account_balance_wallet_outlined, Colors.purple.shade700, Colors.purple.shade50)),
                    ]),
                    ...
                    BasicCard(
                      child: Padding(
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: 'Cari berdasarkan ID transaksi atau nomor invoice...',
                    ...
                    BasicCard(
                      child: Column(children: [
                        ...
                        if (paged.isEmpty)
                          Padding(
                            ...
                            child: Center(child: Column(children: [
                              Icon(Icons.task_alt, size: 56, color: Colors.green.shade300),
                              const SizedBox(height: 12),
                              Text(
                                _searchQuery.isEmpty ? 'Tidak ada pembayaran yang menunggu verifikasi.' : 'Tidak ada hasil untuk "$_searchQuery".',
                                style: TextStyle(color: Colors.grey.shade500, fontSize: 15),
                                textAlign: TextAlign.center,
                              ),
                              if (_searchQuery.isEmpty) ...[
                                const SizedBox(height: 4),
                                Text('Semua pembayaran sudah diproses!', style: TextStyle(color: Colors.grey.shade400, fontSize: 13)),
                              ],
                            ])),
                          )
                        else
                          ...paged.asMap().entries.map((entry) {
                            ...
                            final hasProof = payment.paymentProofUrl != null && payment.paymentProofUrl!.isNotEmpty;
                                ...
                                child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                  ...
                                  Expanded(flex: 3, child: Row(children: [
                                    ...
                                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Text(payment.transactionId ?? 'ID #${payment.id}', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                                      Row(children: [
                                        Icon(hasProof ? Icons.image : Icons.image_not_supported_outlined, size: 13, color: hasProof ? Colors.green.shade600 : Colors.grey.shade400),
                                        const SizedBox(width: 4),
                                        Text(hasProof ? 'Ada bukti' : 'Tanpa bukti', style: TextStyle(fontSize: 11, color: hasProof ? Colors.green.shade600 : Colors.grey.shade400)),
                                      ]),
                                    ]),
                                  ])),

                                  // Invoice
                                  Expanded(flex: 2, child: Text('#${payment.invoiceId}', style: TextStyle(fontSize: 13, color: Colors.grey.shade700))),

                                  // Nominal
                                  Expanded(flex: 2, child: Text(formatRupiah(payment.amount), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14))),

                                  // Metode
                                  Expanded(flex: 2, child: Row(children: [
                                    Icon(_methodIcon(payment.paymentMethod), size: 16, color: _methodColor(payment.paymentMethod)),
                                    const SizedBox(width: 6),
                                    Flexible(child: Text(_methodLabel(payment.paymentMethod), style: TextStyle(fontSize: 12, color: _methodColor(payment.paymentMethod), fontWeight: FontWeight.w600))),
                                  ])),

                                  // Tanggal
                                  Expanded(flex: 2, child: Text(formatDate(payment.paymentDate), style: TextStyle(fontSize: 12, color: Colors.grey.shade600))),
                                  ...
                                  SizedBox(
                                    ...
                                    child: FilledButton(
                                      ...
                                      child: const Text('Cek Bukti', style: TextStyle(fontSize: 12)),
                                    ),
                                  ),
                                ]),
                              ),
                        ...
                        Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
                          child: Row(children: [
                            Text('Menampilkan ${paged.length} dari ${filtered.length} transaksi', style: TextStyle(fontSize: 13, color: Colors.grey.shade500)),
                            const Spacer(),
