  ...
  factory ResidentDetailModel.fromJson(Map<String, dynamic> json) {
    ...
    return ResidentDetailModel(
      ...
      room: ResidentDetailRoom(
        id: _parseNullableInt(roomJson['id'] ?? json['room_id']),
        number: roomJson['number']?.toString() ?? json['room_number']?.toString() ?? '-',
        title: roomJson['title']?.toString() ?? json['room_title']?.toString() ?? '-',
        price: _parseNullableInt(roomJson['price'] ?? json['agreed_price']),
      ),
      resident: ResidentDetailPerson(
        id: tenantJson['user_id']?.toString() ?? json['tenant_user_id']?.toString(),
        name: tenantJson['name']?.toString() ?? json['tenant_name']?.toString() ?? '-',
        email: '-',
        idCardNumber: tenantJson['id_number']?.toString() ?? json['tenant_id_number']?.toString() ?? '-',
        phoneNumber: tenantJson['phone']?.toString() ?? json['tenant_phone']?.toString() ?? '-',
        gender: '-',
        job: '-',
        addressKtp: '-',
